            ...
            <div class="col-lg-8">
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <div>
                            <a href="{{ route('admin.orders.index') }}" class="btn btn-sm btn-outline-secondary">All
                                orders</a>
                        </div>
                    </div>
                                        ...
                                        @foreach ($user->orders()->latest()->limit(10)->get() as $order)
                                            <tr>
                                                ...
                                                <td class="small text-muted">
                                                    {{ optional($order->placed_at)->format('Y-m-d') }}</td>
                                                <td class="text-end">
                                                    {{ number_format($order->grand_total ?? ($order->total ?? 0), 2) }} €
                                                </td>
                                                <td><span
                                                        class="badge bg-{{ $order->status === 'pending' ? 'warning' : ($order->status === 'completed' ? 'success' : 'secondary') }} text-capitalize">{{ $order->status }}</span>
                                                </td>
                                                <td class="text-end">
                        ...
                        <form action="{{ route('admin.users.update', $user) }}" method="POST" id="role-change-form">
                            ...
                            <div class="mb-2">
                                <select name="roles[]" class="form-select"
                                    onchange="document.getElementById('role-change-form').submit();">
