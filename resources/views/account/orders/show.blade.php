...
@section('content')
    <div class="container py-4">
        <div class="d-flex align-items-start justify-content-between mb-3">
            ...
            <div class="text-end">
                <span
                    class="badge bg-{{ $order->status === 'pending' ? 'warning' : ($order->status === 'processing' ? 'info' : ($order->status === 'shipped' ? 'primary' : ($order->status === 'completed' ? 'success' : 'secondary'))) }} text-capitalize">
                    {{ $order->status }}
                </span>
                <div class="mt-2">
            ...
            <div class="col-md-4">
                <div class="card h-100">
                    <div class="card-body">
                        ...
                        <p class="mb-1"><strong
                                class="me-2 text-capitalize">{{ $order->payment_status ?? 'unpaid' }}</strong></p>
                        @if ($order->payments && $order->payments->count())
                            <small class="text-muted">Last payment:
                                {{ optional($order->payments->last())->paid_at ? optional($order->payments->last())->paid_at->toDayDateTimeString() : '-' }}</small>
                    ...
                    <div class="card-body">
                        @if ($order->shippingAddress)
                            ...
                            @if ($order->shippingAddress->phone)
                                <small class="text-muted">Phone: {{ $order->shippingAddress->phone }}</small>
                            @endif
                        @else
                    ...
                    <div class="card-body">
                        @if ($order->billingAddress)
                            ...
                            @if ($order->billingAddress->phone)
                                <small class="text-muted">Phone: {{ $order->billingAddress->phone }}</small>
                            @endif
                        @else
                                ...
                                <tr>
                                    <td>
                                        <div class="d-flex">
                                            ...
                                            @if ($imgUrl)
                                                <img src="{{ $imgUrl }}"
                                                    alt="{{ $img->alt ?? ($item->title ?? '') }}" class="rounded me-3"
