...
@section('content')
    <div class="container py-4">
        ...
        @if ($addresses->count() === 0)
            <div class="alert alert-info">You don't have any saved addresses yet. <a
                    href="{{ route('addresses.create') }}">Create one</a>.</div>
        @endif
                        ...
                        <div class="card-body">
                            ...
                            <address class="mb-2 small">
                                ...
                                @if ($address->line2)
                                    {{ e($address->line2) }}<br>
                                @endif
                                {{ e($address->city) }}@if ($address->state)
                                    , {{ e($address->state) }}
                                @endif
                                <br>
                                @if ($address->postal_code)
                                    {{ e($address->postal_code) }}<br>
                                @endif
                                {{ e($address->country) }}<br>
                                @if ($address->phone)
                                    <strong>Phone:</strong> {{ e($address->phone) }}
                                @endif
                            </address>
