					...
					<div class="card-body">
						<ul class="list-unstyled mb-3">
							...
							<li class="d-flex justify-content-between align-items-start mb-2">
								...
								<div class="text-end">
									<div>{{ number_format(($item->unit_price ?? $item->product->price ?? 0) * $item->qty, 2) }} €</div>
									<div class="small text-muted">{{ number_format($item->unit_price ?? $item->product->price ?? 0, 2) }} € / unit</div>
								</div>
							</li>
							@endforeach
						</ul>
						...
						$taxDisplay = number_format($subtotal * 0.20, 2);
						$grand = number_format($subtotal + ($subtotal >= 100 ? 0 : 5) + ($subtotal * 0.20), 2);
						@endphp

						<div class="d-flex justify-content-between">
							<div class="small text-muted">Shipping</div>
							<div>{{ $shippingDisplay === 'Free' ? 'Free' : number_format($shippingDisplay, 2) . ' €' }}</div>
						</div>
